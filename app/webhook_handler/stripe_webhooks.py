...
async def stripe_webhook(request: Request):
    ...
    try:
        ...
        # Invalid payload
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid payload :"+ str(e))
    except stripe.SignatureVerificationError as e:
        # Invalid signature
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid signature :" + str(e))
...
def handle_customer_created(customer):
    ...
    name = customer.get('name')
    email = customer.get('email')
    webhook_ops.insert_customer(name,email= email, stripe_customer_id= stripe_customer_id)